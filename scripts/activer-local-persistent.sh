#!/usr/bin/env bash
# À sourcer avant d'utiliser le profil persistant local :
#   source scripts/activer-local-persistent.sh

if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
    echo "Ce script doit être sourcé : source scripts/activer-local-persistent.sh" >&2
    exit 1
fi

# Identifiants réservés aux services locaux définis dans compose.local.yaml.
...
# Le profil reste soumis aux mêmes garde-fous que le pilote hébergé. Ces
# valeurs sont exclusivement locales et ne doivent pas être réutilisées.
export CARNET_SECRET_KEY="local-persistent-development-key-do-not-reuse"
